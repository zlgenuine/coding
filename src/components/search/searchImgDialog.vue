<template lang="html">
  ...
  <ts-dialog :close-on-press-escape='false' v-model="dialog.show" width="70%" title="请框选图中要识别的区域" @close="handleClose">
    ...
    <p v-if="showTip" style="color: red; font-size: 14px;text-align: center;margin-top: 10px">该功能只对会员开放，<span @click="toReNew" class="reNew_btn">点击开通</span></p>
  ...
  export default {
    data() {
      return {
        ...
        showTip: false,
        isSecret: window.localStorage.getItem('isSecret') === 'true' && this.$store.state.user.isMemeber
      };
    },
    props: {
      imageUrl: String,
    ...
    watch: {
      ...
      isSecret (val) {
        if (!this.$store.state.user.isMemeber) {
          // this.isSecret = false;
        } else {
          window.localStorage.setItem('isSecret', val);
        }
