<template>
  <div class="enquiry">
    <ts-section>
      <div slot="menu">
        ...
        <ts-table :data="EnquiryList.list">
          <ts-column slot data-key="productPic"  align="center" image name="花型图片"></ts-column>
          <ts-column slot data-key="productNo" width="120px"  align="center" name="花型编号"></ts-column>
          <ts-column slot data-key="companyName" width="100px" align="center" name="档口名称"></ts-column>
          ...
          <ts-column slot  align="center"  name="操作" width="120px" action="[{'text':'查看花型','func':'hanleVisitProduct'},{'text':'档口信息','func':'showenquiryInfo'}]"></ts-column>
      </ts-table>
    ...
    <!-- 对话框 -->
    <ts-dialog v-model="showPersonInfo" :titleNeedCenter="false" title="档口搜花信息" confirmText="访问店铺" @confirm="hanleVisitStore">
        <div class="enquiry-dialog-row">
          <div class="left">账户号码：</div>
          <div class="right">{{enquiryEskUser.phone}}</div>
          <div class="left">姓名：</div>
          <div class="right">{{enquiryEskUser.companyPersonCharge}}</div>
...
<script>
  ...
  export default {
    ...
    async created () {
      this.EnquiryList = (await (listMySearchProduct(this.Params))).data.data;
    },
    watch: {
      // 加工数据
    ...
    methods: {
      ...
      async showenquiryInfo (item) {
        let data = (await getCompanySimpleInfo({
          id: item.companyId
        })).data.data;
