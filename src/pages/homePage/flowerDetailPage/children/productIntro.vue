<template lang="html">
	...
	<div class="productIntro-wrapper">
					...
					<ts-row :gutter="15">
						...
						<ts-col :span="13">
							<div class="productIntro-product-cover">
								<ts-image width="380" height="380" :canView="false" disabledHover :src="Color.picUrl+'?x-oss-process=image/resize,m_fill,h_380,w_380'+watermask">
								</ts-image>
								<div class="productIntro-product-share" @click="handleCollected" v-if="productDetail.userId!==userInfo.id&&userInfo.id">
						...
						<ts-col :span="11">
							...
							<div class="productIntro-product-intro">
								<ts-row>
									<ts-col :span="24" class="productIntro-product-intro--productNo">{{productDetail.productNo}}</ts-col>
								</ts-row>
								<ts-row>
										<ts-row class="productIntro-product-intro--line">
											...
											<ts-col :span="20">{{productDetail.category | filterDict(dicTree.PRODUCT_TYPE,'name')}}&nbsp;</ts-col>
											<ts-col :span="4" v-show="!!productDetail.classList.length">分&nbsp;&nbsp;类：</ts-col>
											<ts-col :span="20" v-show="!!productDetail.classList.length"><span v-for="i in productDetail.classList">{{i}}&nbsp;</span>&nbsp;</ts-col>
											<ts-col :span="4">成&nbsp;&nbsp;分：</ts-col>
											<ts-col :span="20">{{productDetail.ingredient}}&nbsp;</ts-col>
											<ts-col :span="4">货&nbsp;&nbsp;型：</ts-col>
											<ts-col :span="20">{{productDetail.productShape | filterDict(dicTree.PRODUCT_SHAPE,'name')}}&nbsp;</ts-col>
												...
												<div class="productIntro-product-color">
													<div v-if="getColorsLength<2">
														<span v-for="color in Color.list">
																<ts-image :src="cards.picUrl" v-for="(cards,index) in color" height="56" width="56" :canView="false" disabledHover class="productIntro-product-color--image" @click="showColorDetail(cards,index)"  :class="Color.chosed===index?'is-chosed':''" ></ts-image>
														</span>
													</div>
													<div v-if="getColorsLength>=2">
														<ts-carousel class="productIntro-product-color--carousel" v-model="carousel" dots="never" arrow="always" arrowType="square" easing='linear' >
															<ts-carousel-item v-for="color in Color.list">
																<div class="item">
																	<ts-image :src="cards.picUrl" v-for="(cards,index) in color" height="56" width="56" :canView="false" disabledHover @click="showColorDetail(cards,index)" :class="Color.chosed===index?'is-chosed':''" ></ts-image>
																</div>
															</ts-carousel-item>
														</ts-carousel>
													</div>
											...
											<ts-col :span="20">
												...
												<span v-else v-show="!!Color.detail.stock">{{Color.detail.stock}}/{{Color.detail.unit | filterDict(dicTree.PRODUCT_UNIT,'name')}}
												</span>
												&nbsp;
											</ts-col>
										</ts-row>
								...
								<ts-row class="productIntro-product--row">
									...
									<ts-col :span="12" v-if="getIsShowSample" @click.native="Dialog.sendSample=!Dialog.sendSample">
										<div class="productIntro-product--col">
		...
		<!-- 对话框---采购信息登记  -->
		<ts-dialog v-model="Dialog.enquiry" title='采购信息登记' @confirm="handleConfirmEnquiry('enquiryForm')" @cancel="handleCancelEnquiry('enquiryForm')" @close="handleCancelEnquiry('enquiryForm')" width="60%">
			<ts-row :gutter="20">
				<ts-col :span="7">
					<p class="productIntro-dialog-sendSample--title">已选色卡</p>
					<ts-image width="200" height="200" :src="Color.picUrl" :canView="false" disabledHover></ts-image>
				</ts-col>
				<ts-col :span="17">
					<ts-form :model="enquiryForm" :rules="rules" ref="enquiryForm" label-width="125px" label-position="left">
						...
						<ts-form-item label="剪版数量" prop="purchaseNum" v-if="enquiryForm.purchaseType===3">
							<ts-input v-model="enquiryForm.purchaseNum" width="50%"  :maxlength="9" placeholder="请输入剪版数量"></ts-input>
							<ts-select  style="width:30%" data-key-name="name" data-val-name="dicValue" :options="dicTree.PRODUCT_UNIT" v-model="enquiryForm.stockUnit"></ts-select>
						</ts-form-item>
						<ts-form-item label="大货数量" prop="purchaseNum" v-if="enquiryForm.purchaseType===2">
							<ts-input v-model="enquiryForm.purchaseNum" width="50%"  :maxlength="9" placeholder="请输入大货数量"></ts-input>
							<ts-select style="width:30%"  data-key-name="name" data-val-name="dicValue" :options="dicTree.PRODUCT_UNIT" v-model="enquiryForm.stockUnit"></ts-select>
						</ts-form-item>
