<template>
    <div class="list-user">
        ...
        <div class="title title1" v-else>
            ...
            <p class="buyNum">Ta的总求购数：<em>{{obj.totalNum || 0}}个</em></p>
        </div>
        <div class="content">
            <div class="listImg" :class="{'forbideen': e.buyStatus !== 1}" v-for="(e, index) in obj.list"
                 @click="goDetail(e)">
            ...
            <pageBar :showOpt="true" :pageNum="pageNum" :pageMax="pageMax" @upPage="upPage" @downPage="downPage"
                     @selectFirstPage="selectFirstPage" @selectLastPage="selectLastPage"
                     style="margin-right: 20px;"></pageBar>
        </div>
    </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 跳转详情页
      goDetail (e) {
        // buyStatus	求购状态	number	1--求购中 2--已完成 3--已关闭
        if (Number(e.buyStatus) === 1) window.open('/purchaseDetailPage?purchaseId=' + e.id);
      }
    }
  };
</script>
