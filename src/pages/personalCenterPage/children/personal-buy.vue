<template>
<div>
  <ts-title :title="'我的求购'" :userType="2" v-if="titleShow"></ts-title>
  <div class="personal-buy">
    ...
    <div class="personal-buy-wrap clearfix" v-show='!defaultShow'>
      <div class="personal-goods-item" v-for="(item, index) in items">
        ...
        <p><span>求购 <i>{{ item.buyNum || 0 }}</i> {{item.buyUnit | unit}}</span><span class="time">{{item.createDate | customTime}}</span></p>
...
export default {
  ...
  methods: {
    ...
    deleteProductBuyMethod(index) {
      let _ = this;
      _.deleBuyParam.ids = _.items[index].id;
      deleteProductBuy(_.deleBuyParam).then((res) => {
        if (res.data.code === 0) {
          _.closeModel();
          Toast({
            type: 'success',
