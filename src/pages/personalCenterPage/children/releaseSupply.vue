    ...
    <div class="ts-form-box">
      <ts-form :model="addSupplyForm" :rules="rules" ref="addSupplyForm" label-width="125px" label-position="left">
        ...
        <ts-form-item label="供应说明：" prop="supplyDesc">
          <ts-input type="textarea" :rows="4" :maxlength='50' v-model="addSupplyForm.supplyDesc" placeholder="请输入供应说明" style="width: 430px;"></ts-input>
        </ts-form-item>
      </ts-form>
      <div slot="footer">
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async(valid) => {
        if (valid) {
          let res = await releaseCompanySupply(this.addSupplyForm);
          if (!res.data.code) {
            this.$router.push({
